...
export class SolidComponent {
    ...
    public list: IList<ILaboratorySolidDTO> = {
        type: 'نتیجه آزمایش',
        columns: [
            {
                title: 'تاریخ',
                ...
                type: 'DATE',
                action: (data) => ['/solid', 'info', data.id],
            },
        ...
        actions: [
            {
                title: 'مدیریت نتایج',
                icon: 'biotech',
                action: (data: ILaboratorySolidDTO) => ['/solid', 'info', data.id],
            },
            'DIVIDER',
    ...
    loadList(value?: INgxHelperParamValue): void {
        ...
        this.apiService.request<ILaboratorySolidListRs>(
            'LaboratorySolidList',
            { params: { line, cargo, date, page } },
            (response) => {
                this.loading = false;
                this.solids = response.list;
    ...
    create(shift: 'DAY' | 'NIGHT'): void {
        this.ngxHelperBottomSheetService.open(SolidCreateComponent, 'ثبت نتیجه آزمایش درصد جامد', {
            data: { shift, cargos: this.cargos },
        });
    }

    update(solid: ILaboratorySolidDTO): void {
        this.ngxHelperBottomSheetService.open(
            SolidUpdateComponent,
            'ویرایش نتیجه آزمایش درصد جامد',
            { data: { cargos: this.cargos, solid } },
            () => {
                this.loadList();
    ...
    delete(solid: ILaboratorySolidDTO): void {
    ...
    log(solid: ILaboratorySolidDTO): void {
        this.laboratoryTestService.showLog('LaboratorySolidLog', solid.id);
    }
}
